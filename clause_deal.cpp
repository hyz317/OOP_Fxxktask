...
//	if(!cin.getline(cmd,1000,';'))break;//若指令终止，说明一条语句结束，则break
	...
	//	cout<<__FUNCTION__<<__LINE__<<endl; 
		...
		if(_Equal(word[1],"COUNT"))Count(word); //先没区分大小写 
...
//？？ 
	...
	for(auto i:tmp_table.row_map) // 遍历当前 table 中的所有行 
	{
		...
		for(int j=0;j<basic.size();j++)
		{
			...
			{
				if(_Equal(i.second.data[basic[j]],"NULL"))ok=0; // 跳过空值 
			} 
	...
	else if(!_Equal(order_by_attr , "COUNT")) { // 如果有 orderby 且不是 order Count(*)这一列 
